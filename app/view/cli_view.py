...
def get_order_input() -> dict:
    ...
    # Coleta informações dos itens do pedido
    print("\n===== ITENS DO PEDIDO =====")
    items_data = []
...
def display_employee_ranking(ranking_data: list) -> None:
    ...
    """
    # Verificar se é uma mensagem de texto (caso não haja dados)
...
def run_order_creation() -> None:
    ...
    # Chama o controller para criar o pedido
    success, message = OrderController.create_new_order(
...
def run_order_report() -> None:
    ...
    success, data = OrderController.get_order_report(order_id)
    
    if success:
        # Se for bem-sucedido, exibe o relatório formatado
        display_order_report(data)
    else:
        # Se falhar, exibe mensagem de erro
        print(f"\n[ERRO] {data}")

def run_employee_ranking_report() -> None:
    """
    ...
    # Coleta o intervalo de datas
    start_date, end_date = get_date_range_input()
    
    print(f"\nGerando ranking para o período de {start_date} a {end_date}...")
    
    # Chama o controller para gerar o ranking
    success, data = OrderController.get_employee_ranking_report(start_date, end_date)
    
    if success:
        # Se for bem-sucedido, exibe o ranking formatado
